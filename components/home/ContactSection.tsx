import React from 'react';
import { motion } from 'framer-motion';
import { MapPin, Phone, Mail, Clock, Building2 } from 'lucide-react';

export const ContactSection: React.FC = () => {
  return (
    <section id="contact" className="py-24 bg-neutral-50">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="text-center mb-16"
        >
          <h2 className="text-3xl md:text-5xl font-bold text-neutral-900 mb-4">
            Kontakt & Standort
          </h2>
          <p className="text-neutral-600 max-w-2xl mx-auto text-lg">
            Karosserie & Lack und Autoservice – alles an einem Standort in Berlin-Blankenfelde.
          </p>
        </motion.div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          {/* Contact Info */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          >
            {/* "Unter einem Dach" hint */}
            <div className="flex items-center gap-3 mb-8 p-4 bg-white rounded-xl border border-neutral-100">
              <Building2 size={20} className="text-neutral-900 shrink-0" />
              <p className="text-sm text-neutral-600">
                <span className="font-bold text-neutral-900">Unter einem Dach:</span>{' '}
                KFZ Lindner (Karosserie & Lack) und Autoservice Lindner (Service & Mechanik) am selben Standort.
              </p>
            </div>

            <div className="space-y-6">
              <div className="flex items-start gap-4">
                <div className="p-3 bg-white shadow-sm border border-neutral-100 rounded-xl text-neutral-900">
                  <MapPin size={22} />
                </div>
                <div>
                  <h3 className="text-neutral-900 font-bold mb-1">Adresse</h3>
                  <p className="text-neutral-600">
                    Hauptstraße 43<br />
                    13159 Berlin-Blankenfelde
                  </p>
                </div>
              </div>

              <div className="flex items-start gap-4">
                <div className="p-3 bg-white shadow-sm border border-neutral-100 rounded-xl text-neutral-900">
                  <Phone size={22} />
                </div>
                <div>
                  <h3 className="text-neutral-900 font-bold mb-1">Telefon</h3>
                  <p className="text-neutral-600">
                    <a href="tel:0309131252" className="hover:text-black transition-colors">
                      030 / 913 12 52
                    </a>
                    <br />
                    <a href="tel:0309131253" className="hover:text-black transition-colors">
                      030 / 913 12 53
                    </a>
                  </p>
                </div>
              </div>

              <div className="flex items-start gap-4">
                <div className="p-3 bg-white shadow-sm border border-neutral-100 rounded-xl text-neutral-900">
                  <Mail size={22} />
                </div>
                <div>
                  <h3 className="text-neutral-900 font-bold mb-1">E-Mail</h3>
                  <a
                    href="mailto:info@autoservice-lindner.de"
                    className="text-neutral-600 hover:text-black transition-colors"
                  >
                    info@autoservice-lindner.de
                  </a>
                </div>
              </div>

              <div className="flex items-start gap-4">
                <div className="p-3 bg-white shadow-sm border border-neutral-100 rounded-xl text-neutral-900">
                  <Clock size={22} />
                </div>
                <div>
                  <h3 className="text-neutral-900 font-bold mb-1">Öffnungszeiten</h3>
                  <p className="text-neutral-600">
                    Montag – Freitag: 7:00 – 18:00 Uhr
                    <br />
                    und nach Vereinbarung
                  </p>
                </div>
              </div>
            </div>
          </motion.div>

          {/* Map Placeholder */}
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            whileInView={{ opacity: 1, scale: 1 }}
            viewport={{ once: true }}
            className="h-full min-h-[400px] w-full bg-white rounded-2xl overflow-hidden relative border border-neutral-200"
          >
            <div className="absolute inset-0 bg-neutral-100 flex items-center justify-center">
              <div className="text-center">
                <div className="bg-neutral-900 p-4 rounded-full mx-auto mb-4 w-fit">
                  <MapPin className="text-white w-8 h-8" />
                </div>
                <p className="text-neutral-900 font-bold mb-1">Hauptstraße 43</p>
                <p className="text-neutral-500 text-sm">13159 Berlin-Blankenfelde</p>
              </div>
            </div>
            <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur p-3 rounded-xl border border-neutral-100">
              <p className="text-xs text-neutral-500">
                Kartenansicht wird in der Produktion eingebunden.
              </p>
            </div>
          </motion.div>
        </div>
      </div>
    </section>
  );
};
